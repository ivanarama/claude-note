...
# Requires: Python 3.11+ (code uses 3.9+ type hints)
...
py -3.11 -m pip install --quiet pystray Pillow fastapi "uvicorn[standard]" pyinstaller
...
if ($Debug) {
    ...
    py -3.11 -m PyInstaller cn.spec --clean --debug all
} else {
    Write-Host "Building cn.exe..." -ForegroundColor Yellow
    py -3.11 -m PyInstaller cn.spec --clean
}
